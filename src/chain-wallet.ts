import {AnchorProvider, BorshCoder, Instruction, Program} from "@coral-xyz/anchor";
...
export class ChainWalletClient {
    ...
    public getInstructionDataWithNonceWallet(nonce: number, wallet: PublicKey): PublicKey {
        const nonceSeed = new Uint8Array(8)
        const view = new DataView(nonceSeed.buffer)
        view.setBigUint64(0, BigInt(nonce), true) // true = little-endian
     ...
     */
    public async multisigPushInstruction(
        ...
        // 同时去重，避免重复的 pubkey
        const seenKeys = new Map<string, {pubkey: PublicKey, isSigner: boolean, isWritable: boolean}>();
     ...
     */
    public async multisigApprovalRejectInstruction(
        instructionNonce: bigint,
        wallet: PublicKey,
        manager: PublicKey,
     ...
     */
    public async multisigExecuteInstruction(
        ins: TransactionInstruction,
        instructionNonce: bigint,
        wallet: PublicKey,
        manager: PublicKey
